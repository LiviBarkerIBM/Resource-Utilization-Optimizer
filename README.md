...
- Recommendations are deterministic weighted-rule results, not predictions from a hosted machine-learning or generative-AI service.
- Uploaded data is local to the current browser and device.
- There is no shared database, server-side validation, access control, or collaboration workflow.
- Utilization is based on supplied allocations, supplied utilization percentage, or assignment-status fallback; it is not connected to an external time or resource-management system.
- The utilization status filter affects utilization status, not assignment status.
- Detailed allocation values must be valid JSON in the `currentAllocations` cell.
